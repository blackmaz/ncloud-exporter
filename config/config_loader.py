...
class ConfigNamespace:
    ...
    def keys(self):
        """현재 객체가 딕셔너리인 경우 키 목록을 반환"""
        return list(self._data.keys())

    def to_dict(self):
        """ConfigNamespace 객체를 dict로 변환"""
        return {key: self._to_dict(value) for key, value in self._data.items()}

    def _to_dict(self, obj):
        """재귀적으로 ConfigNamespace를 dict로 변환"""
...
class ConfigLoader:
    ...
    VALID_PRODUCTS = ["Server", "LoadBalancer", "Redis", "Nas"]
    ...
    def load_arg_config(self):
        ...
        parser.add_argument(
            ...
            "-p",
            nargs="+",
            help="Product name (Server, LoadBalancer, Redis, Nas)",
            choices=self.VALID_PRODUCTS,
        )
        parser.add_argument(
            "--access_key",
            "-k",
